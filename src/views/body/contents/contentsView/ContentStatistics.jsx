...
class ContentStatistics extends React.Component {
  ...
  print = async () => {
    ...
    MainRepository.Document.getTrackingInfo(document.documentId, (res) => {
      let resData = res;
  ...
  printDetail = async (e, item) => {
    const {document} = this.props;
    MainRepository.Document.getTrackingList(document.documentId, item.cid, item.sid, (res) => {
      let resData = res;
      const resultList = resData.resultList;
      resultList.sort((b, a)=>{
        return b.t - a.t;
