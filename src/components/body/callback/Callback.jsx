...
class Callback extends Component {
  ...
  init = () => {
    ...
    const { setAlertCode, setMyInfo, setMyList, setHistory, location } = this.props;

    // 최초 로그인 시 redux 에 myInfo 저장 필요.
    // callback.jsx 는 Main.jsx 에 route 되기 때문에,
    // Main.jsx 의 init 함수가 끝난 뒤에 호출됨.

    if (MainRepository.Account.isAuthenticated()) return history.push("/");

    MainRepository.Account.handleAuthentication(location).then(sub =>
